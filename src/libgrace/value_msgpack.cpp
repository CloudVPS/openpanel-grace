...
bool value::frommsgpack (const string &m, size_t& offset)
{
    ...
    else switch (opcode)
    {
        ...
        case 0xcf: { unsigned long long s; offset = m.binget64u(offset,s); *this=s;}  break;
        ...
        case 0xd3: { signed long long s; offset = m.binget64(offset,s); *this=s;}  break;
                    
        // no single precision float support in grace. piggyback on int instead
        case 0xca: { union { float s; uint32_t i;}; offset = m.binget32u(offset,i); *this=s;}  break;
        case 0xcb: { double s; offset = m.bingetieee(offset,s); *this=s;}  break;
        ...
        case 0xdc: { // an array up to (2^16)-1 elements. Number of elements is stored in unsigned 16-bit big-endian integer.
                type(t_array);
        ...
        case 0xdd:  { // an array up to (2^32)-1 elements. Number of elements is stored in unsigned 32-bit big-endian integer.
                type(t_array);
        ...
        case 0xde: {
                type(t_dict);
                // a map up to (2^16)-1 elements. Number of elements is stored in unsigned 16-bit big-endian integer.
                uint16_t len; offset = m.binget16u(offset,len);
                value v; // temporary element used for keys
                while(len--)
                {
        ...
        case 0xdf: {
                type(t_dict);
                // a map up to (2^32)-1 elements. Number of elements is stored in unsigned 32-bit big-endian integer.
                uint32_t len; offset = m.binget32u(offset,len);
                value v; // temporary element used for keys
                while(len--)
                {
